...
class UnitModel with ChangeNotifier {
  ...
  void initUnit() async {
    ...
    if (prefs.containsKey(temperatureUnitKey)) {
      temperature =
          TemperatureUnit.values[prefs.getInt(temperatureUnitKey) ?? 0];
    }
    if (prefs.containsKey(windUnitKey)) {
      wind = WindUnit.values[prefs.getInt(windUnitKey) ?? 0];
    }
    if (prefs.containsKey(rainfallUnitKey)) {
      rainfall = RainfallUnit.values[prefs.getInt(rainfallUnitKey) ?? 0];
    }
    if (prefs.containsKey(visibilityUnitKey)) {
      visibility = VisibilityUnit.values[prefs.getInt(visibilityUnitKey) ?? 0];
    }
    if (prefs.containsKey(airPressureUnitKey)) {
      airPressure =
          AirPressureUnit.values[prefs.getInt(airPressureUnitKey) ?? 0];
    }
  }
}
